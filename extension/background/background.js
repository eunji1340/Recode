...
console.log("✅ Recode 확장 프로그램의 백그라운드 서비스 워커가 시작되었습니다.");
...
chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
    if (changeInfo.status === 'complete' && tab.url && tab.url.startsWith("https://www.acmicpc.net")) {
        ...
        chrome.cookies.get({ url: tab.url, name: 'OnlineJudge' }, (cookie) => {
            if (cookie) {
                ...
                console.log("❌ 'OnlineJudge' 쿠키를 찾을 수 없습니다. 백준 로그인이 필요합니다.");
            }
        });
    }
});
...
chrome.cookies.onChanged.addListener((changeInfo) => {
    ...
    if (cookie.name === 'OnlineJudge' && cookie.domain === '.acmicpc.net' && !removed) {
